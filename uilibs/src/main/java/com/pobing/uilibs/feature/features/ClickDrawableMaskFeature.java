...
public class ClickDrawableMaskFeature extends AbsFeature<ImageView> implements CanvasCallback, TouchEventCallback
{
    ...
    public void constructor(Context context, AttributeSet attrs, int defStyle)
    {
        ...
        if (null != attrs)
        {
            TypedArray a = context.obtainStyledAttributes(attrs, com.pobing.uilibs.R.styleable.ClickDrawableMaskFeature);
            if (null != a)
            {
                mClickMaskColor = a.getColor(com.pobing.uilibs.R.styleable.ClickDrawableMaskFeature_uik_clickMaskColor, mClickMaskColor);
                mClickMaskEnable = a.getBoolean(com.pobing.uilibs.R.styleable.ClickDrawableMaskFeature_uik_clickMaskEnable, true);
                a.recycle();
            }
        }
    }
