...
public class CustomCircleProgress extends ProgressBar {
    ...
    //定义一些属性常量
    private static final int PROGRESS_DEFAULT_COLOR = 0xFF8B8B8B;//默认圆(边框)的颜色
    private static final int PROGRESS_REACHED_COLOR = 0XFFFC00D1;//进度条的颜色
    private static final int PROGRESS_REACHED_HEIGHT = 10;//进度条的高度
    private static final int PROGRESS_DEFAULT_HEIGHT = 15;//默认圆的高度
    private static final int PROGRESS_RADIUS = 120;//圆的半径
    ...
    public CustomCircleProgress(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        paintPercent.setTextSize(35);
        paintPercent.setColor(Color.parseColor("#ffffff"));
        paintPercent.setStrokeWidth(20);
        //让三角形的长度等于圆的半径(等边三角形)
        triangleLength = mRadius-50;
        //绘制三角形，首先我们需要确定三个点的坐标
        float firstX = (float) ((mRadius * 2 - Math.sqrt(3.0) / 2 * mRadius) / 2) + 10;//左上角第一个点的横坐标，根据勾三股四弦五定律,Math.sqrt(3.0)表示对3开方
        //为了显示的好看些，这里微调下firstX横坐标
        float mFirstX = (float) (firstX + firstX * 0.2);
        float firstY = mRadius - triangleLength / 2 + 10;
        //同理，依次可得出第二个点(左下角)第三个点的坐标
        float secondX = mFirstX;
        float secondY = (float) (mRadius + triangleLength / 2) + 10;
        float thirdX = (float) (mFirstX + Math.sqrt(3.0) / 2 * mRadius-20) + 10;
        float thirdY = mRadius + 10;
        mPath.moveTo(mFirstX, firstY);
        mPath.lineTo(secondX, secondY);
        mPath.lineTo(thirdX, thirdY);
        mPath.lineTo(mFirstX, firstY);

        setStatus(CustomCircleProgress.Status.INIT);
    }
    ...
    private void setPaint() {
        ...
        mPaintPro = new Paint();
        mPaintPro.setTextSize(20);
        mPaintPro.setColor(getResources().getColor(R.color.white));
...
//        canvas.translate(getPaddingLeft(), getPaddingTop() );
        ...
        canvas.drawArc(new RectF(mDefaultHeight/2, mDefaultHeight/2, getWidth()-mDefaultHeight/2 , getHeight()-mDefaultHeight/2), -90, sweepAngle, false, mPaint);//drawArc：绘制圆弧

        //有了path之后就可以在onDraw中绘制三角形的End和Starting状态了
        if (mStatus == Status.End) {//未开始状态，画笔填充三角形
            //勾的第一个点
            float g_firstX = getWidth() / 4.0f+mDefaultHeight/2;
            float g_firstY = getHeight() / 2.0f;
            mPath2.moveTo(g_firstX, g_firstY);
            //勾的第而个点
            float g_secondX = getWidth() / 2.0f - 10;
            float g_secondY = getHeight() * 0.75f;
            mPath2.lineTo(g_secondX, g_secondY);
            //勾的第三个点
            float g_thirdX = getWidth() * 0.75f + 10;
            float g_thirdY = getHeight() * 0.25f + 10;
            mPath2.lineTo(g_thirdX, g_thirdY);
            mPaint.setStyle(Paint.Style.STROKE);
            ...
            //测量文字长度
            Rect rect = new Rect();
            String str=(int)((float) getProgress()/ getMax() * 100)  + "%";
            paintPercent.getTextBounds(str, 0, str.length(), rect);
            float proLength = rect.width();
            float cx = (mRadius * 2 / 3 + 10 + (2 * mRadius - (mRadius * 2 / 3) + 10)) / 2 - proLength / 2;
            float cy = (2 * mRadius * 2 / 3 + 10 + mRadius * 2 / 3 + 15 + 2 * mRadius * 2 / 3 + 10) / 2;
            canvas.drawText(str, cx, cy, paintPercent);
        } else {//正在进行状态,画两条竖线
            mPaint.setStyle(Paint.Style.STROKE);
            mPaint.setStrokeWidth(dp2px(5));
            mPaint.setColor(Color.parseColor("#ffffff"));
            canvas.drawLine(mRadius * 2 / 3 + 10, mRadius * 2 / 3 + 10, mRadius * 2 / 3 + 10, 2 * mRadius * 2 / 3 + 10, mPaint);
            canvas.drawLine(2 * mRadius - (mRadius * 2 / 3) + 10, mRadius * 2 / 3 + 10, 2 * mRadius - (mRadius * 2 / 3) + 10, 2 * mRadius * 2 / 3 + 10, mPaint);
            //测量文字长度
            Rect rect = new Rect();
            String pro=(int)((float) getProgress() / getMax() * 100 ) + "%";
            paintPercent.getTextBounds(pro, 0, pro.length(), rect);
            float proLength = rect.width();
            float cx = (mRadius * 2 / 3 + 10 + (2 * mRadius - (mRadius * 2 / 3) + 10)) / 2 - proLength / 2;
            float cy = (2 * mRadius * 2 / 3 + 10 + mRadius * 2 / 3 + 15 + 2 * mRadius * 2 / 3 + 10) / 2;
            canvas.drawText(pro, cx, cy, paintPercent);
        }
